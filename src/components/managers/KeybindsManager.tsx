...
export function KeybindsManager() {
    ...
    const allKeybinds = Object.values(avatar.keybinds ?? {}).sort((a, b) => a.name.localeCompare(b.name));
    ...
    const selectedKeybind = selectedKeybindId ? avatar.keybinds?.[selectedKeybindId] : null;
        ...
        <div className="flex flex-col md:flex-row gap-6 h-full">
            ...
            <div className="md:w-1/3 lg:w-1/4 flex-shrink-0 flex flex-col gap-4">
                <div className="flex justify-between items-center pb-3 border-b border-slate-700">
                    ...
                    <Button onClick={handleAddKeybind} className="bg-violet-600 hover:bg-violet-500">
                ...
                <div className="space-y-2 flex-grow overflow-y-auto -mr-2 pr-2">
                    {allKeybinds.map(keybind => (
                        ...
                        >
                            <h3 className="font-semibold text-slate-100 truncate">{keybind.name}</h3>
                            <p className="text-sm text-slate-400 truncate">{keyIdToNameMap.get(keybind.keyId) || keybind.keyId || 'No key set'}</p>
                        </button>
                    ))}
                    {allKeybinds.length === 0 && (
                        <div className="text-center text-slate-500 pt-10">No keybinds configured.</div>
                    )}
                </div>
            </div>
                ...
                {selectedKeybind ? (
                    <div>
                        <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-700">
                            <h2 className="text-2xl font-bold text-slate-100">{selectedKeybind.name}</h2>
                            <Button onClick={() => setDeletingKeybindId(selectedKeybind.uuid)} className="bg-rose-600 hover:bg-rose-500">
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-slate-500">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-16 h-16 mb-4"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><path d="M12 8v8"/><path d="M8 12h8"/></svg>
            ...
            <ConfirmationDialog 
                ...
                message={<>Are you sure you want to delete the <strong>"{deletingKeybindId && avatar.keybinds?.[deletingKeybindId]?.name}"</strong> keybind? This action is permanent.</>}
                variant="danger"
                confirmText="Delete"
            />
        </div>
