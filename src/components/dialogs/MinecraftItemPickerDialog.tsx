...
export function MinecraftItemPickerDialog({ open, onClose, onSelect }: MinecraftItemPickerDialogProps) {
    ...
    return (
        <Dialog open={open} onClose={onClose} className="max-w-4xl">
            ...
            <DialogContent>
                ...
                {loading && <div className="flex h-64 items-center justify-center text-slate-400">Loading items...</div>}
                ...
                <div className="max-h-[60vh] overflow-y-auto -mr-4 pr-4">
                    ...
                    {items && filteredItems.length === 0 && !loading && !error && (
                        <p className="p-4 text-center text-slate-400">No items found for "{filter}".</p>
                    )}
                     {items && filteredItems.length > 300 && (
                        <p className="p-2 text-center text-xs text-slate-500">More than 300 results, please refine your search.</p>
                    )}
                </div>
            </DialogContent>
            <DialogFooter>
                <Button onClick={onClose} className="bg-slate-600 hover:bg-slate-500 focus-visible:ring-slate-400">Close</Button>
            </DialogFooter>
        </Dialog>
    );
}