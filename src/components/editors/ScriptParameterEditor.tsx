...
export function ScriptParameterEditor({ parameters, values, onChange }: ScriptParameterEditorProps) {
    ...
    const renderInputForParam = (param: ScriptDataInstanceParameter) => {
        ...
        switch (param.type.kind) {
            case 'string':
                return <Input type="text" value={value ?? ''} onChange={e => handleValueChange(param.name, e.target.value)} />;
            ...
            case 'ModelPart':
                return (
                    <Select value={value ?? ''} onChange={e => handleValueChange(param.name, e.target.value || undefined)}>
                        <option value="">-- Select a part --</option>
                        {modelElements.map(part => <option key={part} value={part}>{part}</option>)}
                    </Select>
                );
            ...
            case 'ToggleGroup':
                return (
                    <Select value={value ?? ''} onChange={e => handleValueChange(param.name, e.target.value as UUID || undefined)}>
                        <option value="">-- Select a group --</option>
                        {allToggleGroups.map(group => <option key={group.uuid} value={group.uuid}>{group.name}</option>)}
                    </Select>
                );
            ...
            case 'ActionWheel':
                return (
                    <Select value={value ?? ''} onChange={e => handleValueChange(param.name, e.target.value as UUID || undefined)}>
                        ...
                        {allActionWheels.map(wheel => <option key={wheel.uuid} value={wheel.uuid}>{wheel.title}</option>)}
                    </Select>
                );
            
            case 'item':
                return <MinecraftItemPicker value={value ?? ''} onChange={id => handleValueChange(param.name, id)} />;
            
            // TODO: Implement table and list types
            case 'table':
            case 'list':
                 return <div className="text-xs text-amber-400 p-2 bg-amber-900/20 rounded-md">Editing for this complex type is not yet implemented.</div>
            
            default:
                return <div className="text-xs text-rose-400">Unknown parameter type</div>;
        }
    };
            ...
            {parameters.map(param => (
                <FormRow key={param.uuid} label={param.name}>
                    <div>
                        ...
                        {param.helpText && <p className="text-xs text-slate-400 mt-1.5">{param.helpText}</p>}
                    </div>
                </FormRow>
            ))}
        </div>
