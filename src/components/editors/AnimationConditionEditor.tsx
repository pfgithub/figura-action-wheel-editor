...
export function AnimationConditionEditor({
	...
	const activeConditionData = useMemo(() => {
		...
		const activeCondition = getIn({ root: condition }, activeId) as
			| Condition
			| undefined;
		return activeCondition
			? {
					kind: activeCondition.kind,
					label: kindStyles[activeCondition.kind]?.label,
				}
			: null;
	}, [activeId, condition]);
		...
		const updatedCondition = produce(rootCondition, (draft) => {
			...
			if (overId.endsWith(".add")) {
				...
				if (
					...
					(
						container as WritableDraft<ConditionAnd | ConditionOr>
					).conditions.push(nodeToInsert);
				}
			} else {
				const { parentPath: destParentPath, finalKey: destKey } =
					getParentAndFinalKey(overId);
			...
			if (!isPaletteDrag) {
				...
				if (parentPath !== null) {
					...
					if (Array.isArray(parentContainer)) {
						...
						(parentContainer as WritableDraft<ConditionNot>).condition =
							undefined;
