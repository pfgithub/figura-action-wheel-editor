...
interface ActionWheelVisualizerProps {
  ...
  wheelTitle: string;
  onReorder: (oldIndex: number, newIndex: number, preDragSelection: number | null) => void;
}
...
function ActionButton({ action, index, isSelected, onSelectAction }: {
    action: Action;
    index: number;
    isSelected: boolean;
    ...
    return (
        <div ref={ref} {...listeners} {...attributes} className="w-full h-full cursor-grab">
            <button
                onClick={(e) => { e.stopPropagation(); onSelectAction(index); }}
...
export function ActionWheelVisualizer({
  ...
  const [preDragSelectedActionIndex, setPreDragSelectedActionIndex] = useState<number | null>(null);
            ...
            {activeAction ? (
                <div style={{ width: `${BUTTON_SIZE}px`, height: `${BUTTON_SIZE}px` }}>
                    <button
                        ...
                        style={{ backgroundColor: `rgb(${activeAction.color.join(',')})` }}
